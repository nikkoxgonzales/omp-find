...
function keyOf(p: string): { key: string; abs: string | null } {
  ...
  if (scheme !== null && scheme[0].length > 2) {
    if (scheme[0].toLowerCase() !== "file:") return { key: NON_FILE, abs: null };
    try { s = fileURLToPath(s); } catch { return { key: NON_FILE, abs: null }; }
  }
  s = s.replace(/\\/g, "/");
  ...
  if (s === "") return { key: NON_FILE, abs: null };
...
async function tryLock(lockDir: string): Promise<"acquired" | "held" | "giveup"> {
  try {
    await fs.promises.mkdir(lockDir);
    return "acquired";
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code !== "EEXIST") return "giveup";
  }
  try {
    const st = await fs.promises.stat(lockDir);
    if (!st.isDirectory()) return "giveup"; // a file sits at the lock path
...
async function acquireLock(lockDir: string): Promise<boolean> {
  ...
  for (;;) {
    ...
    if (r === "giveup" || Date.now() >= deadline) return false;
...
async function save(store: Store, file: string, merge = false): Promise<void> {
  ...
  try {
    ...
    if (locked) await fs.promises.rm(lockDir, { recursive: true, force: true }).catch(() => undefined);
  }
}
...
export async function recordOpen(p: string): Promise<void> {
  return enqueueWrite(async () => {
    try {
      ...
      if (key === NON_FILE || abs === null) return; // non-file URI — don't write noise
