...
import type { GrepMatch } from "./search.js";

/** Payload handed over via workerData by workerGrep in search.ts. */
interface GrepWorkerData {
  cwd: string;
  pattern: string;
  literal: boolean;
  ignoreCase: boolean;
...
const data = workerData as GrepWorkerData;
try {
  const matches: GrepMatch[] = await walkerRegexGrep(
    data.cwd, data.pattern, data.literal, data.ignoreCase, data.follow,
    Date.now() + data.timeoutMs, data.timeoutMs, data.wholeWord, data.scope,
  );
