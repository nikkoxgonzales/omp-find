...
interface ResultDetails { totalMatched: number; totalFiles: number; truncated: boolean; capped?: boolean }
...
function numParam(params: Record<string, unknown>, key: string): number | undefined {
  const v = params[key];
  if (v === undefined) return undefined;
  if (typeof v !== "number" || !Number.isFinite(v) || !Number.isInteger(v)) throw new Error(`${key} must be an integer >= 1`);
  if (v < 1) throw new Error(`${key} must be >= 1`);
...
function xxhRotl(x: number, r: number): number {
  return ((x << r) | (x >>> (32 - r))) | 0;
}
function xxhRead32(b: Uint8Array, p: number): number {
 ...
 * silently mismatches. */
export function xxh32Bytes(bytes: Uint8Array, seed = 0): number {
  ...
  if (len >= 16) {
    ...
    while (p + 16 <= len) {
      v1 = xxhRound(v1, xxhRead32(bytes, p)); p += 4;
      v2 = xxhRound(v2, xxhRead32(bytes, p)); p += 4;
      v3 = xxhRound(v3, xxhRead32(bytes, p)); p += 4;
      v4 = xxhRound(v4, xxhRead32(bytes, p)); p += 4;
    }
  ...
  h ^= h >>> 15; h = Math.imul(h, XXH_P2) | 0;
  h ^= h >>> 13; h = Math.imul(h, XXH_P3) | 0;
  h ^= h >>> 16;
  return h >>> 0;
}
...
export function hashlineFileHash(text: string): string {
  let body = text.startsWith("\uFEFF") ? text.slice(1) : text;
  if (body.includes("\r")) body = body.replace(/\r\n?/g, "\n");
  const parts = body.split("\n");
  let normalized = "";
  for (let i = 0; i < parts.length; i++) {
    normalized += parts[i].replace(/[ \t\r]+$/, "");
    if (i < parts.length - 1) normalized += "\n";
  }
...
export function hashlineHeader(displayPath: string, text: string): string {
...
async function hashlineTagsFor(cwdDir: string | undefined, files: string[]): Promise<Map<string, string>> {
  const cwd = path.resolve(cwdDir ?? process.cwd());
  const tags = new Map<string, string>();
  await Promise.all([...new Set(files)].map(async (f): Promise<void> => {
    try {
      const buf = await fs.promises.readFile(path.join(cwd, f));
      if (buf.indexOf(0) >= 0) return; // binary skip (same rule as the walker)
...
function groupGrepBlocks(blocks: Array<{ path: string; rows: string[] }>, tags: Map<string, string>): string[] {
  const order: string[] = [];
  const buckets = new Map<string, string[]>();
  for (const b of blocks) {
    let bucket = buckets.get(b.path);
    if (bucket === undefined) { bucket = []; buckets.set(b.path, bucket); order.push(b.path); }
    bucket.push(...b.rows);
  }
  const out: string[] = [];
  for (const p of order) {
    const tag = tags.get(p);
    if (tag !== undefined) out.push(`[${toDisplay(p)}#${tag}]`);
...
function charsParam(params: Record<string, unknown>, key: string): number | undefined {
  ...
  if (!Number.isInteger(v)) throw new Error(`${key} must be an integer >= 1`);
  if (v < 1) throw new Error(`${key} must be >= 1`);
...
export function registerFindTools(pi: any, deps: FindToolsDeps, opts: RegisterFindToolsOptions = {}): void {
  ...
  const grepDef = (toolName: string): Record<string, unknown> => ({
    ...
    execute: async (_toolCallId: string, params: Record<string, unknown>) => {
      ...
      try {
        ...
        let blocks: Array<{ path: string; rows: string[] }>;
        if (concise) {
        ...
        if (lines.length === 0) {
          ...
          const hints: string[] = [];
          if (literal && REGEX_SYNTAX.test(pattern)) hints.push("pattern contains regex syntax; retry with literal:false if a regex was intended");
          if (literal && pattern.trim().split(/\s+/).filter(Boolean).length >= 3) hints.push("try ffjfind with that as query");
  ...
  function jfindRoot(cwd: string, raw: string): string {
  ...
  function keywordsParam(params: Record<string, unknown>): string[] {
    const v = params["grep_keywords"];
    if (v === undefined) return [];
    if (!Array.isArray(v) || v.some(e => typeof e !== "string")) throw new Error("grep_keywords must be an array of strings");
    return (v as unknown[]).filter((e): e is string => typeof e === "string");
  }
  ...
  const jfindDef = (toolName: string): Record<string, unknown> => ({
    description: [
    ...
    execute: async (_toolCallId: string, params: Record<string, unknown>, signal?: AbortSignal, onUpdate?: (update: unknown) => void, ctx?: unknown) => {
      const t0 = Date.now();
      try {
        const query = strictStrParam(params, "query");
        if (query === undefined || query.trim().length === 0) return text(`${toolName} failed: query must be a non-empty description`);
        const q = query.trim();
        ...
        if (rawPath !== undefined) {
          ...
          let st: fs.Stats;
          try { st = fs.statSync(root); } catch { return text(`${toolName} failed: path not found: ${rawPath}`); }
          if (!st.isDirectory()) return text(`${toolName} failed: path is not a directory: ${rawPath}`);
          const relDisp = toDisplay(path.relative(base, root));
          if (relDisp.length > 0) scopeDisplay = `${relDisp}/`;
        }
        ...
        const result = await runCascade({
          ...
          onProgress: (message: string) => {
            try { onUpdate?.({ content: [{ type: "text", text: message }] }); } catch { /* progress is best-effort */ }
          },
          ...(frecency !== undefined ? { frecencyScore: (rel: string) => safeScore(frecency, path.join(root, ...rel.split("/"))) } : {}),
        });
        ...
        if (hits.length === 0) {
          ...
          if (shown.length < hits.length) out.push(`(... ${hits.length - shown.length} more hits not shown; raise limit or refine the query)`);
        }
        out.push(
          "",
          `listed ${stats.listed} · judged ${stats.judged + stats.windowsJudged} · read ${stats.filesRead} files (${humanSize(stats.fileBytes)}) · ${stats.requests} requests · ${fmtNumber(stats.inputTokens)} tokens · $${stats.cost.toFixed(4)} · ${fmtDuration(Date.now() - t0)} wall / ${fmtDuration(stats.apiMs)} api`,
        );
        ...
        if (overBudget(rendered, maxChars)) {
          const byFile = hits.map(hit => [scopeDisplay === undefined ? hit.rel : `${scopeDisplay}${hit.rel}`, hit.linesSeen] as const);
  ...
  const outlineDef = (toolName: string): Record<string, unknown> => ({
    ...
    execute: async (_toolCallId: string, params: Record<string, unknown>) => {
      ...
      try {
        ...
        if (dispForError !== "" && msg.startsWith("outline: cannot read")) return text(`${toolName} failed: outline: cannot read ${dispForError}`);
    ...
    execute: async (_toolCallId: string, params: Record<string, unknown>) => {
      ...
      try {
        ...
        if (rewrite !== undefined) {
          ...
          if (compiled.mode === "references" || compiled.symbolName) return text(`${toolName} failed: rewrite preview needs pattern: or symbol:, not references:`);
