...
- Bash-search guard (`tool_call` on `bash`, `fffind`/`ffgrep`/`ffjfind` are `essential` like `bash`): blocks a leading `grep`/`egrep`/`fgrep`/`rg`, `xargs … grep`, `find … -name/-iname/-path/-ipath/-regex/-wholename` (unless an action flag `-delete`/`-exec`/`-execdir`/`-ok`/`-okdir` is present — fffind can't run actions), and `ls -R`/`ls --recursive`, with a message naming the `ffgrep { "pattern": … }` / `fffind { "pattern": … }` call to use instead. A `| grep`/`| xargs grep` pipe stage only blocks when the first stage produces file contents or listings (`cat`/`head`/`tail`/`ls`/`find`/`dir`/`type`/`sort`/`less`/`more`/`awk`/`sed`/`cut`/`tr`/`uniq`/`jq`) — `tail -f`/`-F`/`--follow` is a live stream and passes, as do `ps aux | grep`, `kubectl get pods | grep`, `history | grep`, `npm test | grep`, `curl … | grep`. `|` inside quotes is masked (`echo "a | grep b"` passes), leading `sudo`/`env`/`nice`/`ionice`/`time`/`command` wrappers and `VAR=value` prefixes are stripped (`FOO=1 grep x` blocks; a flag taking a separate value like `sudo -u root` can still slip through, fail-open), and a grep after `&&`/`||`/`;` never blocks. `OMP_FIND_GUARD=off|0|false|no` disables it; it only runs when the ff tools registered in this process and fails open (returns undefined) on any unexpected shape.
