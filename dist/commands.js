...
function worstKind(a, b) {
    const rank = (k) => (k === "error" ? 2 : k === "warn" ? 1 : 0);
...
function healthText(deps) {
    ...
    let kind = "info";
    try {
        const status = typeof deps?.search?.status === "function" ? deps.search.status() : deps?.search?.status;
        if (status === undefined) {
    ...
    try {
        const fstat = typeof deps?.frecency?.status === "function" ? deps.frecency.status() : deps?.frecency?.status;
        if (fstat === undefined) {
