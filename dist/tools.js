 ...
 */
export function decideBashSearchBlock(command) {
...
function callerCertainty(symbol, rowText, ignoreCase) {
    ...
    const bound = `${l}${esc}${r}`;
    if (new RegExp(`${bound}\\s*\\(`, flags).test(rowText))
        return "exact";
    if (new RegExp(`(?:import|from|require|use|include)\\b[^\n]*${bound}`, flags).test(rowText))
        return "exact";
    return "possible";
}
...
function safeRecordOpen(frecency, p) {
    if (p === undefined)
        return;
    try {
        void Promise.resolve(frecency?.recordOpen(p)).catch(() => undefined);
    }
        ...
        execute: async (_toolCallId, params) => {
            ...
            try {
                ...
                if (cursorId) {
                    ...
                    if (scope !== undefined) {
                        try {
                            filePin = fs.statSync(path.join(path.resolve(cwd ?? process.cwd()), ...scope.split("/"))).isFile();
                        }
                        catch {
                            filePin = false;
                        }
        ...
        execute: async (_toolCallId, params) => {
            ...
            try {
                ...
                if (cursorId) {
                    ...
                    if (depthParam !== undefined && depthParam !== st.depth)
                        diff.push("depth");
                    resumeNote = cursorParamNote(diff);
                }
                else {
