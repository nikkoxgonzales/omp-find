...
async function assertCwdExists(cwd) {
    ...
    if (!st.isDirectory())
        throw new Error(`scan root is not a directory: ${cwd}`);
}
...
function decodeGrepText(buf) {
    ...
    if (buf.length >= 2 && buf[0] === 0xff && buf[1] === 0xfe) {
        ...
        if (swapped.length % 2 === 1)
            return null; // odd byte count: malformed UTF-16BE
        swapped.swap16();
        ...
        if (text.charCodeAt(0) === 0xfeff)
            text = text.slice(1); // UTF-8 BOM: strip so line 1 col 1 matches rg
    }
...
function normalizeScope(scope) {
    ...
    if (segs.length === 0)
        return undefined; // "./" alone: degenerate, not escape-shaped
    ...
    async function walk(dir, rel, depth) {
        ...
        for (const e of entries) {
            ...
            if (isDir) {
                ...
                out.push(relPath); // linked worktrees carry a .git pointer FILE — skip it like the dir
        }
    }
...
export async function findScanned(query, opts = {}) {
    ...
    if (q.gitModifiedOnly) {
        ...
        if (modified === null) {
            ...
            let bare = false;
            try {
                bare = (await runCmd("git", ["rev-parse", "--is-bare-repository"], cwd)).trim() === "true";
            }
...
function byteColToCharCol(text, byteCol) {
    ...
    if (n > Buffer.byteLength(text, "utf8"))
        return byteCol;
...
const patForMsg = (p) => (p.length > 200 ? `${p.slice(0, 200)}…` : p);
...
export async function walkerRegexGrep(cwd, pattern, literal, ignoreCase, follow, deadline = 0, timeoutMs = 0, wholeWord = false, scope) {
    ...
    try {
        ...
        if (wantsU && err instanceof SyntaxError) {
            try {
                ...
                throw new Error(`invalid regex: ${patForMsg(pattern)}`);
            }
        }
        else
            throw new Error(`invalid regex: ${patForMsg(pattern)}`);
    ...
    for (const f of await walkFiles(cwd, follow, deadline, timeoutMs, scope)) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (;;) {
                ...
                if (m[0] === "" && lastNoEol && m.index === lines[i].length)
                    break;
                ...
                if (m[0] === "") {
                    ...
                    if (re.lastIndex > lines[i].length)
                        break;
                } // zero-width rows still emit (rg parity for ^, ^$, z*)
            }
        }
    }
    return out;
...
function workerGrep(cwd, pattern, literal, ignoreCase, follow, timeoutMs, wholeWord, scope) {
    ...
    worker.once("message", (msg) => done(() => (msg && typeof msg.error === "string" ? reject(new Error(msg.error)) : resolve(msg?.matches ?? []))));
...
async function directCallers(sym, base, seen, ring) {
    ...
    const fullLine = async (m) => {
        let lines = lineCache.get(m.path);
        if (lines === undefined) {
            try {
                if ((await fs.stat(path.join(cwdDir, m.path))).size > MAX_GREP_BYTES)
                    lines = null;
                else
                    lines = splitLines(decodeGrepText(await fs.readFile(path.join(cwdDir, m.path))) ?? "");
            }
            catch {
                lines = null;
            }
    ...
    for (let pi = 0; pi < patterns.length; pi++) {
        ...
        for (const m of r.matches) {
            if (v !== null && !v.test(m.text)) {
                ...
                if (line === undefined || !v.test(line))
                    continue;
            }
            if (defRe.test(m.text) || defRe2.test(m.text) || (ctorDefRe !== null && ctorDefRe.test(m.text)) || (ctorBodyRe !== null && ctorBodyRe.test(m.text)))
                continue;
...
export async function capsuleOf(symbol, opts = {}) {
    ...
    for (const m of defHits.matches) {
        if (ordered.length >= 60)
            break;
        if (inScope(m.path.replace(/\\/g, "/")) && !ordered.includes(m.path))
            ordered.push(m.path);
    }
    ...
    const CODE_EXT = { ".ts": true, ".tsx": true, ".mts": true, ".cts": true, ".js": true, ".jsx": true, ".mjs": true, ".cjs": true, ".py": true, ".go": true, ".rs": true, ".java": true, ".cs": true, ".c": true, ".h": true, ".cc": true, ".cpp": true, ".cxx": true, ".hpp": true, ".rb": true, ".php": true, ".swift": true, ".kt": true, ".kts": true, ".scala": true, ".lua": true, ".sh": true, ".bash": true, ".zsh": true, ".pl": true, ".ex": true, ".exs": true, ".erl": true, ".hs": true, ".ml": true, ".fs": true, ".clj": true, ".dart": true, ".vue": true, ".svelte": true };
    const srcTier = (p) => {
        const d = p.replace(/\\/g, "/");
        if (/(?:^|\/)(?:dist|docs?)\//.test(d))
            return false;
    ...
    for (const f of candidates) {
        ...
        let syms;
        try {
            syms = (await outlineFile(f, { cwd, depth: 1 })).symbols;
        }
        catch {
            continue;
        }
