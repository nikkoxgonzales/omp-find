...
export function compileStructural(pattern, opts = {}) {
    ...
    if (pre) {
        ...
        if (at < 0) {
            if (/^(?:inside|has):/.test(rest))
                throw new Error(`${tag}: combinators cannot be chained — got "${rest}"`);
        ...
        function parseSide(side, raw, into) {
            ...
            if (sub === "kind") {
                ...
                if (!base)
                    throw new Error(`unknown structural kind "${arg}" (supported: ${Object.keys(KIND_PATTERNS).join(", ")})`);
        ...
        if (tag === "inside") {
            ...
            if (outerT.references)
                throw new Error(`${tag}: outer operand cannot be references:`);
            return {
        ...
        if (filterT.references)
            throw new Error(`${tag}: filter operand cannot be references:`);
        return {
